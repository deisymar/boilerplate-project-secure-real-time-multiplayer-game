...
import { dimensions, backgroundGame, generateStartPos, avatar } from "./gameConfiguration.mjs";
...
// create new player
let posPlayer = generateStartPos(dimensions.playFieldWidth, dimensions.playFieldHeight, avatar.player);
...
// create bonus Coin
const getNewCoin = () => {    
   let posCoin = generateStartPos(dimensions.playFieldWidth, dimensions.playFieldHeight, avatar.collectibleSprite);
...
const draw = () => {
  ...
  allPlayers.forEach((player) => {
    ...
    allPlayers[winningPlayer].score = localPlayer.score;
            //console.log(allPlayers[winningPlayer]);
            bonus = getNewCoin();
    ...
    //if (!endGame) myRep= requestAnimationFrame(draw);
};
...
socket.on("connected", (connected) => { 
  ...
  console.log(`${connected.msg}, Currently ${connected.connections} player(s)`);
    ...
    if (!endGame) myRep = window.requestAnimationFrame(draw);
...
socket.on("endGameClient", (data) => {
    endGame = data;
    //cancelAnimationFrame(myRep);
});
